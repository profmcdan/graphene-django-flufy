...
DEFAULTS = {
    # Pagination
    "DEFAULT_PAGINATION_CLASS": None,  # 'graphene_django_flufy.paginations.LimitOffsetGraphqlPagination'
...
class GraphQLAPISettings(APISettings):
    MODULE_DOC = "https://github.com/profmcdan/graphene-django-flufy"

    @property
    def user_settings(self):
        if not hasattr(self, "_user_settings"):
            self._user_settings = getattr(settings, "GRAPHENE_DJANGO_FLUFY", {})
...
def reload_graphql_api_settings(*args, **kwargs):
    ...
    if setting == "GRAPHENE_DJANGO_FLUFY":
